...
for Href in $LISTA ;do
       ...
       while [ $FLAG -eq 0 ] ; do
          ...
          if [ $AREA == "sse" ] ; then
              ...
              if [[ ":${HH}" == ":${HH_m03}" ]] ; then
                 erro_laf=0
                 if [[ ":${Href}" == ':00' ]] ; then
                    ${WORKDIR}/scripts/04.3.1_exec_cosmo2cosmo.sh ${AREA} ${HH} ${HSTART} ${Href}
                    erro_laf=$?
                 fi
                 if [[ $erro_laf -eq 0 ]] ; then
                    ${WORKDIR}/scripts/04.3.1_exec_cosmo2cosmo.sh ${AREA} ${HH} ${HSTART} ${Href}
                    erro=$?
                 else
                    exit 201
                 ...
                 ${WORKDIR}/scripts/04.3.1_exec_cosmo2cosmo.sh ${AREA} ${HH} ${HSTART} ${Href} ${HH_m03}
                 erro=$?
              fi
   ...
   #
           ...
           #    dtfile=`/home/gast/libraries/GRIB_API/bin/grib_ls -p dataDate ${WORKDIR}/${AREA1}/data/icondata/data${HH}/ig?ff${FILE}0000 | head -3 | tail -1`
	     	...
	     	if [ ${dtfile}${HH} -ne ${datetimeana} ] ; then
             	   rm -f ${WORKDIR}/${AREA1}/data/icondata/data${HH}/ICONf${FILE}0000
	           FLAG=0
                else
	     	
	     	   ${WORKDIR}/scripts/exec_icon2cosmo-fixbug.sh ${AREA} ${HH} ${HSTART} ${Href}
                   erro=$?
